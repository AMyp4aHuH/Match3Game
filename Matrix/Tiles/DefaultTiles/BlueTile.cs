...
namespace Match3Game.MatrixElements.DefaultTiles
{
    ...
    {
        public BlueTile(TileType type) : base()
        {
            Type = type;
            LoadTextures(type);
        }
        ...
        private void LoadTextures(TileType type)
        {
            ...
            Animation idleAnim = new Animation(TextureProvider.BlueTile.Textures.Idle, AnimationConfig.IdlePeriod, true);
            Animation destroyAnim = new Animation(TextureProvider.BlueTile.Textures.Destroy, AnimationConfig.DestroyPeriod);
            Animation selectAnim = new Animation(TextureProvider.BlueTile.Textures.Destroy, AnimationConfig.SelectPeriod, true);
            Animation waitDestroy = new Animation(TextureProvider.BlueTile.Textures.Idle, AnimationConfig.WhaitDestroy);
